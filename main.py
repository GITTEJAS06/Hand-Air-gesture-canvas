...
blackboard = np.zeros((screen_height, screen_width, 3), dtype=np.uint8)
...
exit_button = ((screen_width - 200, screen_height - 120), (screen_width - 50, screen_height - 50))
clear_button = ((screen_width - 400, screen_height - 120), (screen_width - 250, screen_height - 50))
...
while cap.isOpened():
    ...
    frame = cv2.resize(frame, (screen_width, screen_height))  # Resize frame to fit screen
    ...
    # Modify the drawing logic in the main loop
    if result.multi_hand_landmarks:
        for hand_landmarks in result.multi_hand_landmarks:
            ...
            # Drawing Mode: Only allow drawing when index finger is up and all other fingers are down
            ...
            # Eraser mode when all fingers are down
            if fingers.count(True) == 0:
                cv2.circle(canvas if not use_blackboard else blackboard, index_tip, erase_thickness, (0, 0, 0), -1)
                previous_point = None  # Reset previous point when the hand is closed
            # Drawing mode when only the index finger is up
            elif fingers[1] and not any(fingers[2:]):
                if previous_point is None:
                    previous_point = index_tip
                else:
                ...
                previous_point = None  # Reset previous point when other conditions are met


            # Drawing Logic
            if previous_point is not None:
                if 50 < index_tip[1] < screen_height - 50:  # Avoid buttons
                    cv2.line(canvas, previous_point, index_tip, current_color, brush_thickness)
            previous_point = index_tip
